...
ovid_Samples <- c("ovid03", "ovid06", "ovid09", "ovid12", "ovid15", "ovid18")

for (date in ovid_Samples){
  # 메타데이터 불러오기
  meta_folder <- "./inputs"
  ...
  for (i in seq_along(sample_names)) {
    file_path <- file.path(root, "Samples_ovid/", sample_names[i])
  ...
  tmm_path <- file.path(tmm_folder, paste0(date,"_TMM_ovid.csv"))
  log_tmm_path <- file.path(tmm_folder, paste0(date, "_logTMM_ovid.csv"))
  ...
  deg_path <- file.path(deg_folder, paste0(date, "_DEG_table_ovid.csv"))
  ...
  write.csv(DEG_names, file.path(root, "./DEG_Results/", paste0(date, "_ovid_gene_list.csv")), row.names = FALSE, quote = FALSE)
  ...
  p <- ggplot(Result_df, aes(x = logFC, y = -log10(PValue))) +
    ...
    scale_color_manual(
      values = c("UP" = ControlColor, "DOWN" = ovidColor),
      name = "Group"
    ) +
    theme_bw() +
  ...
  # Save
  ggsave(filename = file.path(root, "./DEG_Results/", date, paste0(date, "_ovid_volcano.pdf")), plot = p, width = 8, height = 8)
}
