...
ovary_Samples <- c("ovary03", "ovary06", "ovary09", "ovary12", "ovary15", "ovary18")

for (date in ovary_Samples){
  # 메타데이터 불러오기
  meta_folder <- "./inputs"
  ...
  for (i in seq_along(sample_names)) {
    file_path <- file.path(root, "Samples_ovary/", sample_names[i])
  ...
  tmm_path <- file.path(tmm_folder, paste0(date,"_TMM_ovary.csv"))
  log_tmm_path <- file.path(tmm_folder, paste0(date, "_logTMM_ovary.csv"))
  ...
  deg_path <- file.path(deg_folder, paste0(date, "_DEG_table_ovary.csv"))
  ...
  write.csv(DEG_names, file.path(root, "./DEG_Results/", paste0(date, "_ovary_gene_list.csv")), row.names = FALSE, quote = FALSE)
  ...
  p <- ggplot(Result_df, aes(x = logFC, y = -log10(PValue))) +
    ...
    scale_color_manual(
      values = c("UP" = ControlColor, "DOWN" = ovaryColor),
      name = "Group"
    ) +
    theme_bw() +
  ...
  # Save
  ggsave(filename = file.path(root, "./DEG_Results/", date, paste0(date, "_ovary_volcano.pdf")), plot = p, width = 8, height = 8)
}
